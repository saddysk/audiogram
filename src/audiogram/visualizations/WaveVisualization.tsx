...
export interface VisualizationPathObject {
  x: number;
  y: number;
}
            ...
            <path
              ...
              d={`M 0 0, ${line
                .map((p, i, pts) => {
                  ...
                  return calcPt(p, prevP as VisualizationPathObject);
                })
                .join(",")}, ${calcPt(
                { x: w, y: 0 },
                lastP as VisualizationPathObject
              )}`}
              stroke={color}
...
export const WaveVisualization: FC<WaveVisualizationProps> = ({
  frequencyData,
  width,
  height,
