...
import { VisualizationPathObject } from "./WaveVisualization";

const rotate = (arr: number[], shift: number) => {
...
const getHills = ({
  ...
  const amplitudes = Array.from({ length: nPoints }).map((_, i) => {
    const sample = samples[(i * sampleStep) % samples.length];
    const processed = processAudioFftValue(sample as number, { maxDb, minDb });
                ...
                <path
                  ...
                  d={`M 0 0, ${line
                    .map((p, i, pts) => {
                      ...
                      const cp1x =
                        (prevP as VisualizationPathObject).x + 0.5 * stepSize;
                      const cp1y = (prevP as VisualizationPathObject).y;
                    ...
                    .join(",")}, C ${
                    (lastP as VisualizationPathObject).x + 0.5 * stepSize
                  } ${(lastP as VisualizationPathObject).y}, ${
                    w - 0.5 * stepSize
                  } 0, ${w} 0`}
                />
              )}
